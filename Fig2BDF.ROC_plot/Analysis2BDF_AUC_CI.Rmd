...
data_INS_invest = INS_mean[,c(1,3:ncol(INS_mean))]
data_BINS_invest = BINS_mean[,c(1,3:ncol(BINS_mean))]
...
### All features
```{r svm_invest_all}
auc_all = auc_boot_cv2(data_all_invest,5,"radial",1000,TRUE,'#000000','data_all_invest_roc_plot.png')
```

### BOLD and FC
```{r svm_invest_BOLD&FC}
auc_bold_fc = auc_boot_cv2(data_BOLD_FC_invest,5,"radial",1000,TRUE,'#ACACAC','data_BOLD_FC_invest_roc_plot.png', ci_span = 0.2)
```

### INS and BINS
```{r svm_invest_INS&BINS}
auc_ins_bins = auc_boot_cv2(data_INS_BINS_invest,5,"radial",1000,TRUE,'#AD91C4','data_INS_BINS_invest_roc_plot.png', ci_span = 0.2)
```

```{r svm_invest_INS&BINS-1, include=FALSE}
auc_ins_bins_1 = auc_boot_cv2(data_INS_BINS_invest,30,"radial",1000,TRUE,'#AD91C4','data_INS_BINS_invest_roc_plot_leave-one-session.png')
```
### BOLD
```{r svm_invest_BOLD}
auc_bold = auc_boot_cv(data_BOLD_invest,5,"radial",1000,TRUE,'#ACACAC','data_BOLD_invest_roc_plot.png')
```

### FC
```{r svm_invest_FC}
auc_fc = auc_boot_cv(data_FC_invest,5,"radial",1000,TRUE,'#ACACAC','data_FC_invest_roc_plot.png')
```

### INS
```{r svm_invest_INS}
auc_ins = auc_boot_cv(data_INS_invest,5,"radial",1000,TRUE,'#AD91C4','data_INS_invest_roc_plot.png')
```

### BINS
```{r svm_invest_BINS}
auc_bins = auc_boot_cv(data_BINS_invest,5,"radial",1000,TRUE,'#AD91C4','data_BINS_invest_roc_plot.png')
```
