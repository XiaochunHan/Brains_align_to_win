...
load_packages(c("dplyr", "caret", "e1071","ggplot2", "pROC", "reshape2", "rstatix","tidyr","stringr","lsr"))
```
...
detailed_stats <- long_data %>%
  group_by(Attack_Defend,L_F,BNS_WNS) %>%
  summarise(
...
model <- aov(mean_value ~ BNS_WNS * Attack_Defend * L_F, data = long_data)
summary(model)
```
