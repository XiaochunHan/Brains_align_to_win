...
rm(list = ls())
source("/Users/xiaochunhan/Documents/Research/Projects/Forcast_group_success/functions_library.R")
...
### reorg data
```{r reorg data_suss}
new <- read_excel('Info_track_coef.xlsx',sheet="raw_data")
tr <- read_excel('DAFC_Behavior_30sess_behav_Fow_Fisher_last.xls',sheet="all")
InfoTrack_Attack_Lead = new[((tr$`role(A=1)` == 1)&(tr$`lead(L=1)`==1)),
                                   c("alpha1","alpha2","beta1")]
InfoTrack_Attack_Follow = tr[((tr$`role(A=1)` == 1)&(tr$`lead(L=1)`==0)),
                                   c("alpha1","alpha2","beta1")]
InfoTrack_Defend_Lead = tr[((tr$`role(A=1)` == 2)&(tr$`lead(L=1)`==1)),
                                   c("alpha1","alpha2","beta1")]
InfoTrack_Defend_Follow = tr[((tr$`role(A=1)` == 2)&(tr$`lead(L=1)`==0)),
                                   c("alpha1","alpha2","beta1")]
groupSuccess = new[((new$roleA1D2 == 1)&(new$leadL1F2==1)), "groupSuccess"]
...
saveRDS(perm_all, "lm_perm_all_5000_r_fisherz.RData")
saveRDS(perm_attack, "lm_perm_attack_5000_r_fisherz.RData")
saveRDS(perm_defend, "lm_perm_defend_5000_r_fisherz.RData")
```

### Permutation read
```{r perm_read_suss}
perm_all = readRDS("lm_perm_all_5000_r_fisherz.RData")
perm_attack = readRDS("lm_perm_attack_5000_r_fisherz.RData")
perm_defend = readRDS("lm_perm_defend_5000_r_fisherz.RData")
```
...
beh_AFW_alpha1 = tr[((tr$`role(A=1)` == 1)&(tr$`lead(L=1)`==0)),c("alpha1")]
beh_AFW_alpha2 = tr[((tr$`role(A=1)` == 1)&(tr$`lead(L=1)`==0)),c("alpha2")]
neural_AFW = new[((new$roleA1D2 == 1)&(new$leadL1F2==0)),grepl('WNC_CH', colnames(new))]
...
saveRDS(perm_AFW_alpha2, "lm_perm_AFW_alpha2_5000_partial_r_fisherz.RData")
